...
// game_logger.move stores `timestamp::now_seconds()`, so values are seconds since epoch.
const DAY_SECONDS = 24n * 60n * 60n;
const ONLINE_WINDOW_SECONDS = 60n * 60n; // "online" = active in the last 60 minutes
...
export async function GET() {
  ...
  // game_logger stores seconds-since-epoch.
  ...
  for (const g of games) {
    ...
    const gameSlug = GAME_NAME[gameTypeNum]?.toLowerCase() === 'wheel'
      ? 'wheel'
  ...
  const sb = await aggregatePlayEventsSince(dayMs);
  const allSb = await aggregatePlayEventsSince(null);
