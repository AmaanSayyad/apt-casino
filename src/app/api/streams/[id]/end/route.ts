...
import { endStreamSession, formatStreamPublic, rewardTierLabel } from '@/lib/server/streamSessions';
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const updated = await endStreamSession(supabase, id);
  if (!updated) return NextResponse.json({ error: 'Could not end session' }, { status: 500 });

  const pub = formatStreamPublic(updated);
  return NextResponse.json({
    stream: pub,
    message:
      pub.rewardTierPct > 0
        ? `Stream ended. Reward tier: ${rewardTierLabel(pub.rewardTierPct)} — pending admin review.`
        : 'Stream ended. Under 5 minutes — not eligible for revenue share.',
  });
}
