...
import DynamicForm, { MinesAutoSessionContext } from "./Form";
...
import { gameData, bettingTableData, winProbabilities } from "./config/gameDetail.jsx";
import { useGameStats } from "@/hooks/useGameStats";
import { getManualFormConfig, getAutoFormConfig } from "./config/formConfig.jsx";
...
import { deriveMinePositions } from '@/lib/provablyFair/solanaFairness';
...
import { createAutoSession, advanceAutoSession, normalizeAutoFormData } from '@/lib/mines/autoBet';
import { aiSettingsToAutoForm } from '@/lib/mines/aiAutoBet';
import {
  loadMinesBetHistory,
  saveMinesBetHistory,
  fetchMinesBetHistoryFromServer,
  computeMinesUserStats,
} from '@/lib/client/minesBetHistory';
...
export default function Mines() {
  ...
  const { symbol, chain: playChain } = usePlayCurrency();
  const fairness = useProvableFairness('mines', playAddress);
  const fairnessRoundRef = useRef(null);
  const autoSessionRef = useRef(null);
  const aiAgentActiveRef = useRef(false);
  const [autoSession, setAutoSession] = useState(null);
  const continueAutoTimeoutRef = useRef(null);

  useEffect(() => {
    return () => {
      if (continueAutoTimeoutRef.current) {
        clearTimeout(continueAutoTimeoutRef.current);
      }
    };
  }, []);

  // Load persisted mines history (localStorage, then Supabase profile API)
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const cached = loadMinesBetHistory(playChain, playAddress);
      if (cached.length > 0) {
        if (!cancelled) setGameHistory(cached);
      } else {
        const fromServer = await fetchMinesBetHistoryFromServer(
          playChain,
          playAddress,
          symbol,
        );
        if (!cancelled && fromServer.length > 0) {
          ...
          saveMinesBetHistory(playChain, playAddress, fromServer);
        }
      }
  ...
  }, [playAddress, playChain, symbol]);

  useEffect(() => {
    if (!playAddress || !historyReadyRef.current) return;
    saveMinesBetHistory(playChain, playAddress, gameHistory);
  }, [gameHistory, playAddress, playChain]);

  const minesUserStats = useMemo(
    () => computeMinesUserStats(gameHistory, symbol),
    [gameHistory, symbol],
  );

  // Real on-chain stats (totalBets / volume / maxWin)
  const { display: minesStatsDisplay, loading: minesStatsLoading } = useGameStats('mines');
  const gameStatistics = {
    totalBets: minesStatsLoading ? '…' : minesStatsDisplay.totalBets,
    totalVolume: minesStatsLoading ? '…' : minesStatsDisplay.volume,
    maxWin: minesStatsLoading ? '…' : minesStatsDisplay.maxWin,
  };
  ...
  // Handle AI agent — starts a real auto-bet session using AI settings
  const startAiAgent = async (settings = aiSettings) => {
    if (!playAddress) {
      toast.error('Connect your wallet to use the AI agent');
      return false;
    }
    if (gameStatus.isPlaying || gameStatus.hasPlacedBet || autoSessionRef.current?.active) {
      toast.info('Wait for the current round to finish first');
      return false;
    }

    const form = aiSettingsToAutoForm(settings, playChain);
    const normalized = normalizeAutoFormData(form);
    const session = createAutoSession(normalized);
    session.startedByAi = true;
    autoSessionRef.current = session;
    setAutoSession({ ...session });
    aiAgentActiveRef.current = true;
    setIsAIActive(true);
    setActiveTab('Auto');

    await launchRound(normalized, { isAutoBetting: true, betOverride: session.currentBet });
    toast.info(
      `AI agent started — ${normalized.mines} mines, ${normalized.tilesToReveal} tiles/round`,
    );
    return true;
  };

  const stopAiAgent = (message, toastFn = toast.info) => {
    aiAgentActiveRef.current = false;
    setIsAIActive(false);
    stopAutoSession(message, toastFn);
  };

  const handleAIToggle = () => {
    if (isAIActive) {
      stopAiAgent('AI agent stopped');
      return;
    }
    void startAiAgent();
  };

  const handleAiStrategyChange = (strategy) => {
    setAISettings((prev) => ({ ...prev, strategy }));
  };

  // Launch a mines round (manual or auto)
  const launchRound = async (formData, { isAutoBetting = false, betOverride = null } = {}) => {
    const payload = isAutoBetting
      ? normalizeAutoFormData({
          ...formData,
          betAmount: betOverride ?? formData.betAmount,
        })
      : formData;
    ...
    if (fairness.enabled) {
      ...
      minePositions = deriveMinePositions(fairnessRound.seedBytes, 5, minesCount);
    }

    setBetSettings({
      ...payload,
      betAmount: betOverride ?? payload.betAmount,
      mines: Number(payload.mines) || 5,
      tilesToReveal: Number(payload.tilesToReveal) || 5,
      aiAssist: Boolean(payload.aiAssist),
      isAutoBetting,
      minePositions,
      roundId: Date.now(),
    });
  ...
  const stopAutoSession = (message, toastFn = toast.info) => {
    if (continueAutoTimeoutRef.current) {
      clearTimeout(continueAutoTimeoutRef.current);
      continueAutoTimeoutRef.current = null;
    }
    ...
    setAutoSession(null);
    if (message) toastFn(message);
  };

  const scheduleNextAutoRound = (session) => {
    if (continueAutoTimeoutRef.current) {
      clearTimeout(continueAutoTimeoutRef.current);
    }
    continueAutoTimeoutRef.current = setTimeout(() => {
      continueAutoTimeoutRef.current = null;
      void launchRound(session.settings, {
        isAutoBetting: true,
        betOverride: session.currentBet,
      });
    }, 500);
  };

  const handleAutoRoundFailed = (errorMessage) => {
    if (aiAgentActiveRef.current) {
      stopAiAgent(
        `AI agent stopped: ${errorMessage || 'Could not place bet'}`,
        toast.error,
      );
      return;
    }
    stopAutoSession(
      `Auto bet stopped: ${errorMessage || 'Could not place bet'}`,
      toast.error,
    );
  };
  ...
  const handleFormSubmit = async (formData) => {
    ...
    if (isAutoBetting) {
      const normalized = normalizeAutoFormData({
        ...formData,
    ...
    stopAutoSession(null);
    await launchRound(formData, { isAutoBetting: false });
  };

  // Handle AI settings save
  const handleAISettingsSave = (newSettings) => {
    setAISettings(newSettings);
    if (isAIActive && !gameStatus.isPlaying && !gameStatus.hasPlacedBet && !autoSessionRef.current?.active) {
      void startAiAgent(newSettings);
    }
  };
  ...
  const handleGameComplete = async (result) => {
    ...
    try {
      ...
      console.error('Fairness reveal failed:', error);
    } finally {
      fairness.reset();
      fairnessRoundRef.current = null;
    }

    const newHistoryItem = {
      id: `${Date.now()}-${Math.floor(Math.random() * 1000000)}`,
      mines: result.mines || 0,
      bet: `${result.betAmount || 0} ${symbol}`,
      outcome: result.won ? 'win' : 'loss',
      payout: result.won ? `${result.payout || 0} ${symbol}` : `0 ${symbol}`,
    ...
    if (playAddress && result.betAmount > 0) {
    ...
    if (session?.active) {
      const { stop, reason } = advanceAutoSession(session, result);
      setAutoSession({ ...session });

      if (stop) {
        if (session.startedByAi || aiAgentActiveRef.current) {
          aiAgentActiveRef.current = false;
          setIsAIActive(false);
        }
        if (reason === 'profit') {
          stopAutoSession(
            `Auto stopped — profit target hit (+${session.sessionPnL.toFixed(4)} ${symbol})`,
            toast.success,
          );
        } else if (reason === 'loss') {
          stopAutoSession(
            `Auto stopped — loss limit reached (${session.sessionPnL.toFixed(4)} ${symbol})`,
            toast.info,
          );
        } else {
          stopAutoSession(
            `Auto finished — ${session.totalRounds} rounds (${session.sessionPnL >= 0 ? '+' : ''}${session.sessionPnL.toFixed(4)} ${symbol})`,
            toast.info,
          );
        }
        return;
  ...
  const renderMainContent = () => (
    <div className="flex flex-col lg:flex-row gap-4 site-page-pad-x">
      ...
      <div className="w-full lg:w-1/3 xl:w-1/4">
        ...
        >
          <MinesAutoSessionContext.Provider value={autoSession}>
            <Tabs tabs={tabs} onTabChange={handleTabChange} />
          </MinesAutoSessionContext.Provider>
        </motion.div>
      </div>
  ...
  const renderGameInfo = () => (
    <div className="mt-10 site-page-pad-x">
      ...
      <motion.div
        ...
        transition={{ duration: 0.5, delay: 0.3 }}
      >
        {typeof MinesHistory === 'function' && (
          <MinesHistory gameHistory={gameHistory} userStats={minesUserStats} />
        )}
      </motion.div>
  ...
  return (
    <motion.div className="site-game-page bg-[#070005] bg-gradient-to-b from-[#070005] to-[#0e0512] text-white mines-bg custom-scrollbar">
      ...
      <AIAutoBetting
        ...
        onStrategyChange={handleAiStrategyChange}
