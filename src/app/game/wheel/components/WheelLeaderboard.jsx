...
const WheelLeaderboard = () => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      abortRef.current = ac;
      setLoading(true);
      const res = await fetch('/api/leaderboard?game=wheel&metric=pnl&period=all&top=5', {
        signal: ac.signal,
        cache: 'no-store',
      });
    ...
    <Paper
      ...
      sx={{
        ...
        '&::before': {
          ...
          width: '100%',
          height: '5px',
          background: 'linear-gradient(90deg, #d82633, #FFA500)',
        },
      }}
    >
      <Typography
      ...
      <Box>
        ...
        {!loading && !error && rows.length === 0 && (
          <Box sx={{ textAlign: 'center', py: 4, color: 'rgba(255,255,255,0.6)' }}>
            <Typography variant="body2">No Wheel rounds played yet.</Typography>
            <Typography variant="caption" sx={{ display: 'block', mt: 1, color: 'rgba(255,255,255,0.4)' }}>
              Be the first — your wallet appears here once you spin the wheel.
            </Typography>
          </Box>
        )}
      ...
      >
        ...
        <Box
          component={Link}
          href="/leaderboard?game=wheel&metric=pnl&period=all"
          sx={{
            display: 'flex',
            alignItems: 'center',
            gap: 0.5,
